...
/*   Updated: 2023/02/08 13:27:01 by seojo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_wall(t_map *map, double x, double y)
{
	...
	if (x <= 0 || y <= 0 || x >= map->width * MINI_SCALE || y >= map->height * MINI_SCALE)
		return (1);
	xX = (int)floor(x / MINI_SCALE);
	yY = (int)floor(y / MINI_SCALE);
...
{
	if (hv->wall_hit == 1)
		hv->distance = d_b_points((map->player->x * MINI_SCALE), (map->player->y * MINI_SCALE), hv->x_hit, hv->y_hit);
	else
		hv->distance = DBL_MAX;
}
...
void	cal_ray(t_map *map, t_rtmp *hv)
{
	...
	n_touch_x = hv->xinter;
	n_touch_y = hv->yinter;
	while (n_touch_x >= 0 && n_touch_x <= (map->width * MINI_SCALE) && n_touch_y >= 0 && n_touch_y <= (map->height * MINI_SCALE))
	{
...
void	horz_ray(t_map *map, t_rtmp *horz)
{
	...
	horz->x_hit = 0;
	horz->y_hit = 0;
	horz->yinter = floor(map->player->y * MINI_SCALE / (MINI_SCALE)) * MINI_SCALE;
	horz->yinter += map->ray->facing_down ? MINI_SCALE : 0;
	horz->xinter = (map->player->x * MINI_SCALE) + (horz->yinter - (map->player->y * MINI_SCALE)) / tan(map->ray->ray_angle);
...
void	vert_ray(t_map *map, t_rtmp * vert)
{
	...
	vert->x_hit = 0;
	vert->y_hit = 0;
	vert->xinter = floor(map->player->x * MINI_SCALE / (MINI_SCALE)) * MINI_SCALE;
	vert->xinter += map->ray->facing_right ? MINI_SCALE : 0;
	vert->yinter = (map->player->y * MINI_SCALE) + (vert->xinter - (map->player->x * MINI_SCALE)) * tan(map->ray->ray_angle);
